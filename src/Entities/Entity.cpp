...
void cEntity::TakeDamage(cEntity & a_Attacker)
{
	...
	cEnchantments Enchantments = a_Attacker.GetEquippedWeapon().m_Enchantments;
	...
	else if (BaneOfArthropodsLevel > 0)
	{
		if (IsMob())
		{
			...
			switch (Monster->GetMobType())
			{
				...
				{
					RawDamage += 2.5 * BaneOfArthropodsLevel;
					break;
				}
			}
		}
